...
export default function ChurchFinderSection({ config }: ChurchFinderSectionProps) {
  ...
  const handleSearch = async (coords?: { lat: number; lng: number }, locationName?: string, source: 'search' | 'geolocation' = 'search') => {
    ...
    try {
      ...
      let churchesWithDistance = churches.map(church => {
        const lat = church.coordinates?.lat ?? 0;
        const lng = church.coordinates?.lng ?? 0;
        return {
          ...church,
  ...
  const handleChurchSelect = (church: Church | null) => {
    ...
    if (church) {
      setMapCenter(church.coordinates || { lat: 0, lng: 0 });
  ...
  return (
    <section 
      className="py-24 bg-gradient-to-br from-blue-100/60 via-sky-50/40 to-blue-200/50 dark:from-blue-950/30 dark:via-blue-900/20 dark:to-blue-800/30"
      data-section="church-finder"
    >
